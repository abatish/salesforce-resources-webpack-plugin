...
describe('resource-builder', () => {
  ...
  describe('addPage', () => {
    it('should create a component for default settings, when pageType is component', () => {
      const assets = {
        'appjs.resource': {},
        'appcss.resource': {}
      };
      ...
      builder.options.component.templateOptions.defaultScripts = [ "https://test.com" ];
      builder.options.pageType = 'component';

      builder.addPage(assets, [], originalTypes);
      expect(Object.keys(assets).length).to.be.equal(3);
      expect(assets['components/index_component.component']).to.be.a('object');
      expect(assets['components/index_component.component'].source()).to.contain('URLFOR($Resource.appresources, \'appjs\')');
      expect(assets['components/index_component.component'].source()).to.contain('URLFOR($Resource.appresources, \'appcss\')');
      expect(assets['components/index_component.component'].source()).to.contain('<script src="https://test.com" type="text/javascript"></script>');
    });
