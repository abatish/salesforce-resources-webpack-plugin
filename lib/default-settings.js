    ...
    template: `<apex:component
      ...
      <% if(options.attributes) { %>
        ...
        <% options.attributes.forEach(function(attr) { %>
          <% if(options.attributeWindowVar) { %>
            <script>
            window["JSENCODE(<%= attr.name %>)"] = '{! JSENCODE(<%= attr.name %>) }';
            </script>
          <% } %>
          <apex:attribute
            name="<%= attr.name %>"
            type="<%= attr.type %>"
            <%= typeof(attr.description) != 'undefined' ? 'type="'+ attr.description +'"': '' %>
            <%= typeof(attr.assignTo) != 'undefined' ? 'type="'+ attr.assignTo +'"': '' %>
            <%= typeof(attr.required) != 'undefined' ? 'type="'+ attr.required +'"': '' %>
            <%= typeof(attr.default) != 'undefined' ? 'type="'+ attr.required +'"': '' %>
          />
        <% }) %>
      <% } %>
